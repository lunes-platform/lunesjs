...
export async function blockSignature(signature: string):Promise<IBlock> {
    const response = await axios.get(`https://lunesnode.lunes.io/blocks/signature/${signature}`);
    return response.data;
}
...
*/
export async function blockAddress(address: string, from: number, to: number):Promise<any> {
    const response = await axios.get(`https://lunesnode.lunes.io//blocks/address/{address}/{from}/{to}`);
    return response.data;
}